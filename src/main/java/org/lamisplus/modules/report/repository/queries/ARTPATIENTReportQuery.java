...
public class ARTPATIENTReportQuery {
    ...
    public static final String ART_PATIENT_REPORT_QUERY =  "WITH bio_data AS (\n" +
            ...
            "INNER JOIN (\n" +
            "  SELECT * FROM \n" +
            "(SELECT p.id, CASE WHEN address_object->>'city' IS NOT NULL THEN CONCAT_WS(' ', address_object->>'city',  REPLACE(REPLACE(COALESCE(NULLIF(address_object->>'line', '\\\\\\\\'), ''), ']', ''), '[', ''), \n" +
            "NULLIF(NULLIF(address_object->>'stateId', 'null'), '')) ELSE NULL END AS address, CASE WHEN address_object->>'stateId' ~ '^[0-9.]+$' THEN address_object->>'stateId' ELSE NULL  END AS stateId,\n" +
            ...
            " ep.display AS entryPoint, visit_date as dateOfRegistration, ROW_NUMBER() OVER (PARTITION BY h.person_uuid ORDER BY visit_date DESC ) rnkk\n" +
            ...
            "FROM\n" +
            "    laboratory_order lo\n" +
            "        INNER JOIN hiv_enrollment_commencement h ON h.person_uuid = lo.patient_uuid\n" +
            ...
            "WHERE lo.archived = 0 AND h.archived = 0 AND lo.facility_id = ?1\n" +
            "ORDER BY lo.patient_uuid, lo.order_date DESC\n" +
            " ),\n" +
            " pharmacy_details AS (\n" +
            ...
            " INNER JOIN (SELECT hap.person_uuid, MAX(hap.visit_date) AS MAXDATE FROM hiv_art_pharmacy hap\n" +
            " INNER JOIN hiv_enrollment_commencement h ON h.person_uuid=hap.person_uuid  WHERE h.archived=0\n" +
            ...
            " INNER JOIN hiv_enrollment_commencement he ON he.person_uuid=r.person_uuid\n" +
            ...
            "FROM hiv_status_tracker hst INNER JOIN hiv_enrollment_commencement h ON h.person_uuid=person_id\n" +
            "GROUP BY person_id\n" +
            ...
            " WHERE he.archived=0 AND hartp.archived=0 AND hartp.facility_id=?1 \n" +
            " ORDER BY hartp.person_uuid ASC\n" +
            " ),\n" +
            ...
            " CASE WHEN h.cd4_at_art_start=0 THEN null ELSE h.cd4_at_art_start END  AS baseLineCd4,\n" +
            " CASE WHEN h.cd4_percentage=0 THEN null ELSE h.cd4_percentage END AS cd4Percentage,\n" +
            " h.visit_date as artStartDate\n" +
            " FROM triage_vital_sign tvs\n" +
            " INNER JOIN hiv_enrollment_commencement h ON tvs.uuid=h.vital_sign_uuid AND h.person_uuid=tvs.person_uuid\n" +
            " INNER JOIN patient_person p ON p.uuid=h.person_uuid\n" +
            " RIGHT JOIN hiv_regimen hv ON hv.id=h.regimen_id\n" +
            " RIGHT JOIN hiv_regimen_type hrt ON hrt.id=h.regimen_line_id\n" +
            " RIGHT JOIN base_application_codeset clinical_stage ON clinical_stage.code=h.clinical_stage_id\n" +
            " -- RIGHT JOIN base_application_codeset func_status ON func_status.id=h.functional_status_id\n" +
            " WHERE h.archived=0 AND h.facility_id=?1\n" +
            " ),\n" +
            "current_clinical AS (\n" +
            ...
            "NULL AS baseLineFunctionalStatus,\n" +
            "a.firstRegimen,\n" +
            "a.firstRegimenLine,\n" +
            "a.baseLineCd4,                      \n" +
            "a.cd4Percentage,                          \n" +
