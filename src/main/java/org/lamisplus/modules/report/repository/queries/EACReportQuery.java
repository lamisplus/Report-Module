...
public class EACReportQuery {
    ...
    public static final String EAC_REPORT_QUERY = "with eac_clients as ( " +
            ...
            "), " +
            "eac_count as ( " +
            "    select person_uuid, count(*) as numberOfEACSessionsCompleted from ( " +
            "    with current_eac as ( " +
            "      select person_uuid, uuid, ROW_NUMBER() OVER (PARTITION BY person_uuid ORDER BY id DESC) AS row " +
            ...
            "vl_unsuppressed as ( " +
            "    select " +
            "        fuvl.person_uuid, fuvl.result_reported as dateOfUnsuppressedVLR, " +
            ...
            "LEFT JOIN third_eac ON eac_clients.patientId = third_eac.person_uuid " +
            "LEFT JOIN fourth_eac ON eac_clients.patientId = fourth_eac.person_uuid " +
            "LEFT JOIN fifth_eac ON eac_clients.patientId = fifth_eac.person_uuid " +
            "LEFT JOIN sixth_eac ON eac_clients.patientId = sixth_eac.person_uuid " +
            "LEFT JOIN seventh_eac ON eac_clients.patientId = seventh_eac.person_uuid " +
            "LEFT JOIN eighth_eac ON eac_clients.patientId = eighth_eac.person_uuid " +
            "LEFT JOIN ninth_eac ON eac_clients.patientId = ninth_eac.person_uuid " +
            "LEFT JOIN eac_count ON eac_clients.patientId = eac_count.person_uuid " +
            "LEFT JOIN post_eac_vl1 ON eac_clients.patientId = post_eac_vl1.person_uuid " +
            "LEFT JOIN post_eac_vl2 ON eac_clients.patientId = post_eac_vl2.person_uuid " +
            "LEFT JOIN regimen_at_start ON eac_clients.patientId = regimen_at_start.person_uuid " +
            "LEFT JOIN last_pick ON eac_clients.patientId = last_pick.person_uuid " +
            "LEFT JOIN vl_unsuppressed ON eac_clients.patientId = vl_unsuppressed.person_uuid";
}