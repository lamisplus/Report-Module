...
public interface ReportRepository extends JpaRepository<Report, Long> {
        ...
        @Query(value = "WITH bio_data AS (SELECT DISTINCT (p.uuid) AS personUuid,p.hospital_number AS hospitalNumber, h.unique_id as uniqueId,\n"
                        ...
                        +
                        // "then (select name from base_organisation_unit where id = cast(addr as int))
                        ...
                        +
                        // "select uuid AS personUuid,
                        // (jsonb_array_elements(address->'address')->>'district') as addr from
                        // patient_person \n" +
                        ") dt)," +
                        "\n" +
                        ...
                        "       LEFT JOIN client_verification cvl on cvl.person_uuid = bd.personUuid ", nativeQuery = true)
