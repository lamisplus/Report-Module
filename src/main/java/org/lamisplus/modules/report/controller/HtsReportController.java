...
public class HtsReportController {
	...
	public void htsLineList(HttpServletResponse response, @RequestParam("facilityId") Long facility,
							...
							@RequestParam("endDate") LocalDate end, @RequestParam("reportType") String reportType,
                            @RequestParam("scramber") Boolean scramber
                            ) throws IOException {
		messagingTemplate.convertAndSend(Constants.REPORT_GENERATION_PROGRESS_TOPIC, "Starting HTS report");

		ByteArrayOutputStream baos = generateExcelService.generateHts(facility, start, end, reportType, scramber);
