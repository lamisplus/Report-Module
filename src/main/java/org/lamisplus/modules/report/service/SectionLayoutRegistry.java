// SectionLayoutRegistry.java — Java 8 compatible
...
/**
 * Maps ReportSectionDTO#sectionType -> SectionLayoutDef.
 *
 * This class holds NO report-specific knowledge. Each report module
 * registers its own layouts (typically via a @PostConstruct in a small
 * per-report "layouts" component - see HtsSectionLayouts / PrepSectionLayouts
 * for examples). Adding a new report type is a new registrant, not a change
 * here.
 *
 * If a sectionType has no registered layout, callers should fall back to
...
public class SectionLayoutRegistry {
    ...
    private final Map<String, SectionLayoutDef> layouts = new ConcurrentHashMap<>();
    ...
    public void registerAll(Map<String, SectionLayoutDef> toRegister) {
        for (Map.Entry<String, SectionLayoutDef> entry : toRegister.entrySet()) {
    ...
    public boolean has(String sectionType) {
        return find(sectionType) != null;
    }
}
//package org.lamisplus.modules.report.service;
//
