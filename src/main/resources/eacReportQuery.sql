-- replace 
-- facility_id = 1959
-- NOW()::DATE to your prefared date
...
eac_count as ( 
    ...
    select hes.person_uuid from hiv_eac_session hes 
        join current_eac ce on ce.person_uuid = hes.person_uuid where ce.row = 1 and hes.archived = 0 
            and hes.eac_session_date between '1980-01-01' and NOW()::DATE 
...
post_eac_vl1 as ( 
    ...
    vl AS ( 
        ...
        LEFT JOIN laboratory_result lr ON lr.test_id = lt.id AND lr.date_result_reported BETWEEN '1980-01-01' AND NOW()::DATE 
       ...
       SELECT hap.person_uuid, hap.visit_date AS dateOfStartOfRegimenBeforeUnsuppressedVLR, 
            ...
            FROM hiv_art_pharmacy hap 
             ...
             INNER JOIN public.hiv_regimen_type rt on rt.id = r.regimen_type_id 
                     WHERE r.regimen_type_id in (1,2,3,4,14) and hap.visit_date between '1980-01-01' and NOW()::DATE 
...
vl_unsuppressed as ( 
    ...
    vl as ( 
        SELECT 
            ...
            FROM laboratory_test lt 
            LEFT JOIN laboratory_sample ls ON ls.test_id = lt.id AND ls.archived = 0 
            LEFT JOIN laboratory_result lr ON lr.test_id = lt.id AND lr.date_result_reported BETWEEN '1980-01-01' AND NOW()::DATE and lr.archived = 0 
