-- replace facility_id = 1959
-- replace hac.visit_date >= 
-- hac.visit_date < NOW()::DATE
-- between '1980-01-01' and NOW()::DATE 
with bio_data as (
    SELECT  facility_lga.name AS lga, p.other_name, p.surname, p.first_name, 
      ...
      WHERE 
          h.archived = 0 
        AND p.archived = 0 
        AND h.facility_id = 1959
        AND hac.is_commencement = TRUE 
        AND hac.visit_date >= '1980-01-01' 
        AND hac.visit_date < NOW()::DATE
), 
tb_status as ( 
    with tbscreening_cs as ( 
        ...
        WHERE type = 'Chronic Care' and data is not null and archived = 0 
            and date_of_observation between '1980-01-01' and NOW()::DATE 
            and facility_id = 1959
        ) 
    ...
    tbscreening_hac as ( 
        with h as (
            ...
            where h.archived = 0 and h.visit_date between '1980-01-01' and NOW()::DATE  and facility_id = 1959 
        ) 
...
tb_treatement_start as ( 
    with tbt as ( 
        ...
        FROM public.hiv_observation WHERE type = 'Chronic Care' AND facility_id = 1959 and archived = 0 
    ) 
...
tb_treatement_completion as ( 
  select person_uuid, tb_treatment_outcome, tb_completion_date from (select CAST(data->'tptMonitoring'->>'treatmentOutcome' AS text) AS tb_treatment_outcome, 
         ...
         ROW_NUMBER() OVER ( PARTITION BY person_uuid ORDER BY date_of_observation DESC) AS row_number 
FROM public.hiv_observation WHERE type = 'Chronic Care' AND facility_id = 1959
...
current_tb_result AS ( 
    with cur_tb as ( 
        ...
        WHERE 
            ...
            AND sm.facility_id = 1959 
            AND sm.date_result_reported <= NOW()::Date
    ) 
...
ipt_start as ( 
    with tpt as ( 
        ...
        where hap.archived = 0 and hap.facility_id = 1959
