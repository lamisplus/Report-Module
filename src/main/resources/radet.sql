WITH bio_data AS (
    ...
    WHERE
            h.archived = 0
      AND h.facility_id = 1873
      AND hac.is_commencement = TRUE
      AND hac.visit_date >= '1985-01-01'
      AND hac.visit_date < '2023-04-13'
),
...
sample_collection_date AS (
	 SELECT CAST(sample.date_sample_collected AS DATE ) as DateOfViralLoadSampleCollection, patient_uuid as person_uuid120  FROM (
		    ...
		    AND  lt.viral_load_indication !=719
                       ...
                       AND date_sample_collected <= '2023-04-13'
                       AND sample.facility_id = 1873 ),
...
current_vl_result AS (SELECT * FROM (
                       ...
                       AND  vl_result.dateOfCurrentViralLoad <= '2023-04-13'
                       AND  vl_result.vlFacility = 1873
                       ),
    ...
    careCardCD4 AS (SELECT visit_date, coalesce(cast(cd_4 as varchar), cd4_semi_quantitative) as cd_4, person_uuid AS cccd4_person_uuid
	   ...
	   AND  archived = 0
	   AND  cd_4 != 0
	   AND visit_date <= '2023-04-13'
	   AND facility_id = 1873
	   ),
 ...
 labCD4 AS (SELECT * FROM (
            ...
            AND sm. date_result_reported IS NOT NULL
	        AND sm.archived = 0
	        AND sm.facility_id = 1873
	        AND sm.date_result_reported <= '2023-04-13'
            )as cd4_result
 ...
 pharmacy_details_regimen AS (
	...
	from (
			SELECT p.person_uuid as person_uuid40, p.dsd_model_type as dsdModel, p.visit_date as lastPickupDate,
                       ...
                       AND  p.archived = 0
                       AND  p.facility_id = 1873
                       AND  p.visit_date >= '1985-01-01'
                       AND  p.visit_date  < '2023-04-13'
		) as pr1
		) as pr2
		where pr2.rnk3 = 1
      ),
...
ovc AS (
 ...
 previous_previous AS (
           ...
           AND stat.status_date > pharmacy.visit_date THEN stat.hiv_status
                       ...
                       (
                       CASE
                       WHEN hp.visit_date + hp.refill_period + INTERVAL '28 day' < '2022-12-31' THEN 'IIT'
                       ELSE 'ACTIVE'
                       END
                       ) status,
                       (
                       CASE
                       WHEN hp.visit_date + hp.refill_period + INTERVAL '28 day' < '2022-12-31'  THEN hp.visit_date + hp.refill_period + INTERVAL '28 day'
                       ELSE hp.visit_date
                       END
                       ) AS visit_date,
                       hp.person_uuid
           ...
           FROM hiv_status_tracker WHERE archived=0 AND status_date <= '2022-12-31' )s
           WHERE s.row_number=1
                       ) hst
                       INNER JOIN hiv_enrollment he ON he.person_uuid = hst.person_id
                       WHERE hst.status_date <= '2022-12-31'
    ...
    previous AS (
                      ...
                      AND stat.status_date > pharmacy.visit_date THEN stat.status_date
                       ...
                       (
                       CASE
                       WHEN hp.visit_date + hp.refill_period + INTERVAL '28 day' < '2023-03-31' THEN 'IIT'
                       ELSE 'ACTIVE'
                       END
                       ) status,
                       (
                       CASE
                       WHEN hp.visit_date + hp.refill_period + INTERVAL '28 day' <  '2023-03-31' THEN hp.visit_date + hp.refill_period + INTERVAL '28 day'
                       ELSE hp.visit_date
                       END
                       ) AS visit_date,
                       hp.person_uuid
           ...
           SELECT * FROM (SELECT DISTINCT (person_id) person_id, status_date, cause_of_death,
           hiv_status, ROW_NUMBER() OVER (PARTITION BY person_id ORDER BY status_date DESC)
           FROM hiv_status_tracker WHERE archived=0 AND status_date <=  '2023-03-31' )s
           WHERE s.row_number=1
                       ) hst
                       INNER JOIN hiv_enrollment he ON he.person_uuid = hst.person_id
                       WHERE hst.status_date <=  '2023-03-31'
                       ) stat ON stat.person_id = pharmacy.person_uuid
                       ),
     ...
     current_status AS (
                       ...
                       (
                       CASE
                       WHEN hp.visit_date + hp.refill_period + INTERVAL '28 day' < '2023-04-13' THEN 'IIT'
                       ELSE 'ACTIVE'
                       END
                       ) status,
                       (
                       CASE
                       WHEN hp.visit_date + hp.refill_period + INTERVAL '28 day' < '2023-04-13' THEN hp.visit_date + hp.refill_period + INTERVAL '28 day'
                       ELSE hp.visit_date
                       END
                       ) AS visit_date,
                       hp.person_uuid
           ...
           SELECT * FROM (SELECT DISTINCT (person_id) person_id, status_date, cause_of_death,
           hiv_status, ROW_NUMBER() OVER (PARTITION BY person_id ORDER BY status_date DESC)
           FROM hiv_status_tracker WHERE archived=0 AND status_date <= '2023-04-13' )s
           WHERE s.row_number=1
                       ) hst
                       INNER JOIN hiv_enrollment he ON he.person_uuid = hst.person_id
                       WHERE hst.status_date <= '2023-04-13'
                       ) stat ON stat.person_id = pharmacy.person_uuid
                       ),
...
naive_vl_data AS (
           ...
           WHERE hap.archived=0 AND hrt.id IN (1,2,3,4,14) AND hap.facility_id = 1873 ) pharm
           ...
           WHERE ls.archived=0 AND ls.facility_id=1873
...
SELECT DISTINCT ON (bd.personUuid) personUuid AS uniquePersonUuid,
    ...
    AND bd.artstartdate + 91 < '2023-04-13') THEN TRUE
    WHEN (nvd.age >= 15
    AND nvd.regimen !='TDF-3TC-DTG'
    AND bd.artstartdate + 181 < '2023-04-13') THEN TRUE
    WHEN (nvd.age <= 15 AND bd.artstartdate + 181 < '2023-04-13') THEN TRUE
    ...
    AND CAST(bd.artstartdate AS DATE) + 181 < '2023-04-13' THEN TRUE

    WHEN CAST(NULLIF(REGEXP_REPLACE(cvlr.currentviralload, '[^0-9]', '', 'g'), '') AS INTEGER) IS NULL
    AND scd.dateofviralloadsamplecollection IS NOT NULL AND
    cvlr.dateofcurrentviralload IS NULL
    AND CAST(bd.artstartdate AS DATE) + 91 < '2023-04-13' THEN TRUE
    ...
    AND CAST(cvlr.dateofcurrentviralload AS DATE) + 181 < '2023-04-13' THEN TRUE

    WHEN  CAST(NULLIF(REGEXP_REPLACE(cvlr.currentviralload, '[^0-9]', '', 'g'), '') AS INTEGER) < 1000
    AND (scd.dateofviralloadsamplecollection > cvlr.dateofcurrentviralload
    OR cvlr.dateofcurrentviralload IS NULL
    )
    AND CAST(scd.dateofviralloadsamplecollection AS DATE) + 91 < '2023-04-13' THEN TRUE
    ...
    AND CAST(cvlr.dateofcurrentviralload AS DATE) + 91 < '2023-04-13' THEN TRUE
    ...
    AND bd.artstartdate + 91 < '2023-04-13')
    THEN CAST(bd.artstartdate + 91 AS DATE)
    WHEN (nvd.age >= 15
    AND nvd.regimen !='TDF-3TC-DTG'
    AND bd.artstartdate + 181 < '2023-04-13')
    THEN CAST(bd.artstartdate + 181 AS DATE)
    WHEN (nvd.age <= 15 AND bd.artstartdate + 181 < '2023-04-13')
    THEN CAST(bd.artstartdate + 181 AS DATE)

    WHEN CAST(NULLIF(REGEXP_REPLACE(cvlr.currentviralload, '[^0-9]', '', 'g'), '') AS INTEGER) IS NULL
    AND scd.dateofviralloadsamplecollection IS NULL AND
    cvlr.dateofcurrentviralload IS NULL
    AND CAST(bd.artstartdate AS DATE) + 181 < '2023-04-13' THEN
    CAST(bd.artstartdate AS DATE) + 181

    WHEN CAST(NULLIF(REGEXP_REPLACE(cvlr.currentviralload, '[^0-9]', '', 'g'), '') AS INTEGER) IS NULL
    AND scd.dateofviralloadsamplecollection IS NOT NULL AND
    cvlr.dateofcurrentviralload IS NULL
    AND CAST(bd.artstartdate AS DATE) + 91 < '2023-04-13' THEN
    CAST(bd.artstartdate AS DATE) + 91

    WHEN CAST(NULLIF(REGEXP_REPLACE(cvlr.currentviralload, '[^0-9]', '', 'g'), '') AS INTEGER) < 1000
    AND( scd.dateofviralloadsamplecollection < cvlr.dateofcurrentviralload
    OR  scd.dateofviralloadsamplecollection IS NULL )
    AND CAST(cvlr.dateofcurrentviralload AS DATE) + 181 < '2023-04-13'
    ...
    AND CAST(scd.dateofviralloadsamplecollection AS DATE) + 91 < '2023-04-13' THEN
    ...
    AND CAST(cvlr.dateofcurrentviralload AS DATE) + 91 < '2023-04-13' THEN
